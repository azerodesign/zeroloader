...
async function tiktokScraper(url) {
  try {
    ...
    if (!data || !data.data) throw new Error('Gagal mengambil data dari source.');
    
    const $ = cheerio.load(data.data);
    
    const result = {
      status: true,
      title: $('.tik-video .thumbnail .content h3').text().trim() || 'TikTok Video',
      thumbnail: $('.tik-video .thumbnail img').attr('src'),
      video_nowatermark: $('.tik-video .download-box li a.download-link').eq(0).attr('href'),
      video_watermark: $('.tik-video .download-box li a.download-link').eq(1).attr('href'),
      audio: $('.tik-video .download-box li a.download-link').last().attr('href')
    };
    return result;
  } catch (error) {
...
<body class="selection:bg-cyan-500 selection:text-black">
                ...
                <div class="relative border-t border-b border-cyan-500/50 bg-black/60 backdrop-blur-md">
                    ...
                    <div class="p-8 flex flex-col md:flex-row gap-8">
                        ...
                        <div class="flex-1">
                            ...
                            <div id="download-options" class="grid gap-2"></div>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <script>
        ...
        initBtn.addEventListener('click', async () => {
            ...
            try {
                // FETCH KE API DIRI SENDIRI (Relative Path)
                ...
                if (!resJson.status) throw new Error(resJson.message || "Failed to fetch data");
        ...
        function renderResult(data) {
            ...
            const createBtn = (label, url) => {
                if(!url) return;
                const div = document.createElement('div');
                div.className = "flex items-center justify-between p-3 bg-cyan-900/10 border-l-2 border-cyan-900";
                div.innerHTML = \`<div class="text-cyan-500 font-bold text-sm">\${label}</div><a href="\${url}" target="_blank" class="px-4 py-1.5 bg-transparent border border-cyan-600 text-cyan-400 font-bold text-xs">DOWNLOAD</a>\`;
                optsContainer.appendChild(div);
            };
            createBtn('NO WATERMARK', data.video_nowatermark);
            createBtn('WATERMARK', data.video_watermark);
            createBtn('AUDIO', data.audio);
