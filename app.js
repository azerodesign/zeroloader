...
// Inisialisasi Client (Token diambil dari Environment Variable Vercel)
// Jangan hardcode token di sini biar aman!
const client = new ApifyClient({
    token: process.env.APIFY_TOKEN 
});

// --- LOGIKA APIFY (Serverless Wrapper) ---
async function tiktokScraper(url) {
  try {
    console.log('[APIFY] Memulai scraping untuk:', url);

    // 1. Jalankan Actor (clockworks/free-tiktok-scraper)
    // Actor ini spesifik buat TikTok, lebih simpel daripada web-scraper biasa
    const run = await client.actor("clockworks/free-tiktok-scraper").call({
        postUrls: [url],
        commentsPerPost: 0,
        shouldDownloadCovers: false,
        shouldDownloadSlideshowImages: false,
        shouldDownloadVideos: false // Kita cuma butuh link-nya, gak perlu download filenya ke Apify storage
    });

    console.log('[APIFY] Run selesai, mengambil dataset:', run.defaultDatasetId);

    // 2. Ambil hasil dari dataset
    const { items } = await client.dataset(run.defaultDatasetId).listItems();

    if (!items || items.length === 0) {
        throw new Error('Apify tidak menemukan data video (Mungkin Private/Dihapus).');
    }

    const videoData = items[0]; // Ambil item pertama
    ...
    return {
      ...
      thumbnail: videoData.videoCover || videoData.authorMeta?.avatar,
      // Apify biasanya ngasih link video asli (tanpa WM) di properti 'videoUrl' atau 'playAddr'
      video_nowatermark: videoData.videoUrl || videoData.playAddr, 
      video_watermark: videoData.downloadAddr, // Kadang sama aja tergantung API TikTok
      audio: videoData.musicMeta?.playUrl
    };
...
app.post('/api/scrape', async (req, res) => {
  ...
  if (!process.env.APIFY_TOKEN) {
      return res.status(500).json({ status: false, message: 'SERVER_ERROR: APIFY_TOKEN belum disetting di Vercel.' });
  }
    ...
    switch (targetPlatform) {
        ...
        default:
            return res.status(200).json({ 
                status: false, 
                message: `MODULE [${targetPlatform.toUpperCase()}] MAINTENANCE.` 
            });
    }
...
// --- FRONTEND HTML (Sama seperti sebelumnya, versi v4.3) ---
const FRONTEND_HTML = `
<!DOCTYPE html>
<html lang="id">
<head>
    ...
    <nav class="border-b border-cyan-900/30 bg-black/80 backdrop-blur-md sticky top-0 z-30">
        <div class="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
            <div class="flex items-center gap-3 cursor-pointer group" onclick="window.location.reload()">
                ...
                <div class="flex flex-col"><span class="text-xl font-bold tracking-tighter text-white font-sans">ZERO<span class="text-cyan-400">LOADER</span></span><span class="text-[9px] tracking-[0.4em] text-cyan-600 uppercase">Sys v4.3 APIFY</span></div>
            </div>
        </div>
    </nav>
    ...
    <main class="relative z-10 max-w-6xl mx-auto px-4 pt-16 pb-32">
        <div class="flex flex-col items-center">
            <div class="relative mb-12 text-center">
                <div class="inline-flex items-center gap-3 px-4 py-1 mb-6 border border-cyan-500/30 bg-cyan-950/20 rounded-none text-xs tracking-[0.2em]"><div class="w-1.5 h-1.5 bg-cyan-400 shadow-[0_0_10px_cyan]"></div>SYSTEM_READY // APIFY_LINKED</div>
                <div class="relative group"><h1 class="text-5xl md:text-8xl font-black text-white mb-2 tracking-tighter relative z-10 glitch-text">ZERO_LOADER</h1></div>
    ...
    <script>
        ...
        function runLogs() {
            ...
            const interval = setInterval(() => {
                ...
                i++;
            }, 800); // Agak dilamain karena Apify butuh waktu
        }
        ...
        function renderResult(data) {
            ...
            createBtn('WATERMARK', data.video_watermark);
            createBtn('AUDIO ONLY', data.audio);
