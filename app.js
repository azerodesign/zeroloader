...
async function tiktokScraper(url) {
  try {
    ...
    if (!data || data.code !== 0) throw new Error('Gagal mengambil data dari TikWM.');
    ...
    return {
      ...
      video_nowatermark: videoData.play, 
      video_watermark: videoData.wmplay,
...
// --- API ROUTE UTAMA (MULTI-PLATFORM) ---
app.post('/api/scrape', async (req, res) => {
  ...
  try {
    ...
    // Switch logic untuk routing platform
    switch (platform) {
        case 'tiktok':
        ...
        case 'instagram':
        case 'twitter':
            // Simulasi response untuk platform yang belum aktif
            return res.status(501).json({ 
                status: false, 
                message: `MODULE_${platform.toUpperCase().replace('-','_')}_OFFLINE // MAINTENANCE` 
            });
        default:
            return res.status(400).json({ status: false, message: 'UNKNOWN_PLATFORM_PROTOCOL' });
    }
...
<head>
    ...
    <title>ZERO_LOADER | MULTI-CORE</title>
...
<body class="selection:bg-cyan-500 selection:text-black">
    ...
    <nav class="border-b border-cyan-900/30 bg-black/80 backdrop-blur-md sticky top-0 z-30">
        <div class="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
            <div class="flex items-center gap-3 cursor-pointer group" onclick="window.location.reload()">
                ...
                <div class="flex flex-col"><span class="text-xl font-bold tracking-tighter text-white font-sans">ZERO<span class="text-cyan-400">LOADER</span></span><span class="text-[9px] tracking-[0.4em] text-cyan-600 uppercase">Sys v4.1 MULTI</span></div>
            </div>
        </div>
    </nav>
    ...
    <main class="relative z-10 max-w-6xl mx-auto px-4 pt-16 pb-32">
        <div class="flex flex-col items-center">
            <div class="relative mb-12 text-center">
                <div class="inline-flex items-center gap-3 px-4 py-1 mb-6 border border-cyan-500/30 bg-cyan-950/20 rounded-none text-xs tracking-[0.2em]"><div class="w-1.5 h-1.5 bg-cyan-400 shadow-[0_0_10px_cyan]"></div>SYSTEM_READY // MULTI_CORE_ONLINE</div>
                ...
                <div class="relative bg-[#050505] border border-cyan-800/50 p-1 md:p-2 shadow-[0_0_50px_-10px_rgba(6,182,212,0.15)]">
                    <div class="bg-cyan-950/5 border border-cyan-900/30 p-6 md:p-8 relative overflow-hidden">
                        ...
                        <div class="grid grid-cols-3 md:grid-cols-5 gap-2 mb-8">
                            <button onclick="selectPlatform('tiktok')" id="btn-tiktok" class="btn-platform active flex flex-col items-center justify-center p-3 border border-cyan-900/50 rounded transition-all">
                                <i data-lucide="music" class="w-5 h-5 mb-1"></i> <span class="text-[9px] font-bold tracking-widest">TIKTOK</span>
                            </button>
                            <button onclick="selectPlatform('yt-video')" id="btn-yt-video" class="btn-platform flex flex-col items-center justify-center p-3 border border-cyan-900/50 rounded transition-all">
                                <i data-lucide="video" class="w-5 h-5 mb-1"></i> <span class="text-[9px] font-bold tracking-widest">YT_VIDEO</span>
                            </button>
                            <button onclick="selectPlatform('yt-music')" id="btn-yt-music" class="btn-platform flex flex-col items-center justify-center p-3 border border-cyan-900/50 rounded transition-all">
                                <i data-lucide="headphones" class="w-5 h-5 mb-1"></i> <span class="text-[9px] font-bold tracking-widest">YT_MUSIC</span>
                            </button>
                            <button onclick="selectPlatform('instagram')" id="btn-instagram" class="btn-platform flex flex-col items-center justify-center p-3 border border-cyan-900/50 rounded transition-all">
                                <i data-lucide="instagram" class="w-5 h-5 mb-1"></i> <span class="text-[9px] font-bold tracking-widest">INSTAGRAM</span>
                            </button>
                            <button onclick="selectPlatform('twitter')" id="btn-twitter" class="btn-platform flex flex-col items-center justify-center p-3 border border-cyan-900/50 rounded transition-all">
                                <i data-lucide="twitter" class="w-5 h-5 mb-1"></i> <span class="text-[9px] font-bold tracking-widest">TWITTER</span>
                            </button>
                        </div>
            ...
            <!-- RESULT SECTION -->
            <div id="result-section" class="w-full max-w-4xl mt-12 hidden animate-[slideUp_0.5s_ease-out]">
                <div class="relative border-t border-b border-cyan-500/50 bg-black/60 backdrop-blur-md">
                    ...
                    <div class="p-8 flex flex-col md:flex-row gap-8">
                        <div class="w-full md:w-72 flex-shrink-0 relative">
                            <div class="relative h-40 bg-black border border-cyan-700 overflow-hidden">
                                <img id="res-thumb" src="" alt="thumb" class="w-full h-full object-cover opacity-80" />
                            </div>
                        </div>
                        <div class="flex-1">
    ...
    <script>
        lucide.createIcons();
        // Ubah endpoint ke /api/scrape agar lebih umum
        ...
        function selectPlatform(platform) {
            ...
            const placeholders = {
                ...
                'yt-video': 'PASTE YOUTUBE VIDEO URL...',
                'yt-music': 'PASTE YOUTUBE MUSIC URL...',
                'instagram': 'PASTE INSTAGRAM URL...',
                'twitter': 'PASTE TWITTER/X URL...'
            };
        ...
        function runLogs() {
            ...
            const steps = ['> INITIALIZING ' + currentPlatform.toUpperCase() + '_MODULE...', '> ESTABLISHING CONNECTION...', '> BYPASSING SECURITY...', '> PARSING DATA...', '> DONE.'];
            let i = 0;
        ...
        initBtn.addEventListener('click', async () => {
            ...
            try {
                // KIRIM DATA URL DAN PLATFORM KE BACKEND
        ...
        function renderResult(data) {
            ...
            const createBtn = (label, url) => {
                ...
                if (!url) {
                    div.innerHTML = \`<div class="text-red-400 font-bold text-sm tracking-widest">\${label}</div><button disabled class="w-full sm:w-auto px-6 py-3 bg-red-900/20 border border-red-900/50 text-red-500 font-bold text-xs tracking-widest uppercase rounded cursor-not-allowed">UNAVAILABLE <i data-lucide="x-circle" class="w-4 h-4 ml-2 inline"></i></button>\`;
                } else {
            ...
            // Mapping berdasarkan platform (Untuk sekarang TikTok dulu yang full support)
